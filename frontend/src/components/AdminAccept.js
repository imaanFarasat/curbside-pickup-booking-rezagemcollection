...
import { CheckCircle, User, Calendar, Clock, Mail, Phone, Package, MessageSquare } from 'lucide-react';
...
const AdminAccept = () => {
  ...
  const fetchBooking = async () => {
    try {
      const response = await axios.get(`/api/admin/accept/${token}`);
  ...
  const handleAccept = async () => {
    setProcessing(true);
    try {
      await axios.get(`/api/admin/accept/${token}`);
      toast.success('Booking accepted successfully! Customer has been notified.');
      setTimeout(() => {
        navigate('/');
      }, 2000);
    } catch (error) {
      ...
      <div className="bg-white rounded-lg shadow-lg p-6">
        <div className="text-center mb-6">
          ...
          <p className="text-gray-600">
            Review the booking details below and confirm acceptance
          </p>
        </div>
        ...
        <div className="space-y-6">
          ...
          <div className="flex flex-col sm:flex-row gap-4">
            <button
              ...
              disabled={processing}
              className="flex-1 bg-green-600 text-white py-3 px-6 rounded-lg font-semibold hover:bg-green-700 focus:outline-none focus:ring-2 focus:ring-green-500 disabled:bg-gray-400 disabled:cursor-not-allowed transition-colors"
            >
              {processing ? (
          ...
          <div className="bg-blue-50 rounded-lg p-4">
            <h4 className="font-medium text-blue-900 mb-2">What happens when you accept:</h4>
            <ul className="text-sm text-blue-800 space-y-1">
              <li>• The customer will receive a final confirmation email</li>
              <li>• The time slot will be confirmed and locked</li>
              <li>• The booking status will be updated to "Confirmed"</li>
              <li>• You can track this booking in your admin dashboard</li>
            </ul>
          </div>
        </div>
      </div>
